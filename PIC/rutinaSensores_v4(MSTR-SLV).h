/*
SE AÑADIERON VARIABLES, MANEJADAS EN EL MAIN, QUE PERMITEN EL ENVIO DE LAS CUENTAS INDIVIDUALES DE SUBIDA Y BAJADA DE CADA POSTE 15/10/19
SE CAMBIO LA LA VARIABLE QUE SE GUARDA EN EL ESPACIO DE MEMORIA 0X0005, PASAJEROS TOTALES (SLV+MSTR), 
POR LA SUMA DE PASAJEROS QUE SUBEN Y BAJAN DE AMBOS POSTES DIVIDA EN 2 ---24/10/19
*/
...
void muestraEstatus(){
  if(MSTR){
   ...
   lcd_out(1, 18, auxS);
   /*inttostr(lecturaTablaS, auxS);
   auxCuentaS = ltrim(auxS);
   lcd_out(1, 1, auxCuentaS);*/
   ...
   lcd_out(2, 18, auxB);  
   /*inttostr(lecturaTablaB, auxB);
   auxCuentaB = ltrim(auxB);
   lcd_out(2,16, auxCuentaB);*/
   lecturaTablaPVerdaderos = eepromLeeNumero(0x0005, 2);
   bytetostr(lecturaTablaPVerdaderos, auxP);
   lcd_out(4, 17, auxP);
   /*inttostr(lecturaTablaPVerdaderos, auxP);
   auxCuentaT = ltrim(auxP);
   lcd_out(4, 16, auxCuentaT);*/
   lecturaBloqueo = eepromLeenumero(0x000B, 1) + eepromLeeNumero(0x000C, 1);
   bytetostr(lecturaBloqueo, auxBloq);
   lcd_out(3, 18, auxBloq);
   /*subenSlv = eepromLeeNumero(0x0900, 2);
   bytetostr(subenSlv, auxslv);
   lcd_out(4,7,auxslv);*/
  }
  else if(!MSTR){
   lcd_out(3, 8, "SLV");
   ...
   /*inttostr(lecturaTablaS, auxS);
   auxCuentaS = ltrim(auxS);
   lcd_out(1,19,auxCuentaS);*/
   lecturaTablaB = eepromLeeNumero(0x0003,2);
   bytetostr(lecturaTablaB, auxB);
   lcd_out(2, 16, auxB);
   /*inttostr(lecturaTablaB, auxB);
   auxCuentaB = ltrim(auxB);
   lcd_out(2,19,auxCuentaB);*/
   lecturaTablaPasajerosTotales = eepromLeeNumero(0x0009, 2);
   bytetostr(lecturaTablaPasajerosTotales, auxT);
   lcd_out(4, 18, auxT);
   /*inttostr(lecturaTablaPasajerosTotales, auxT);
   auxCuentaT = ltrim(auxT);
   lcd_out(4,19, auxCuentaT);*/
