...
unsigned int encuentraDireccion(char *nombreTablaEscribir, char *nombreColumnaEscribir, int filaEscribir, unsigned int *direccionFilasOcupadas){
    ...
    for(i = 0;i < objBaseDatos.tablasTotales;i++){
        ...
        if(!strncmp(nombreTablaEscribir, objBaseDatos.nombreTabla, strlen(nombreTablaEscribir))){
            objBaseDatos.direccionDinamica += DOSBYTE + MaximoNombreTabla + BYTE;
            *direccionFilasOcupadas = objBaseDatos.direccionDinamica;
            objBaseDatos.direccionDinamica += CUATROBYTE;
            ...
            for(aux = 1;aux <= objBaseDatos.columnasTotales;aux++){
                /*shorttostr(aux,debug);
                lcd_out(4,1,debug);
                //delay_ms(500);
                shorttostr(objBaseDatos.columnasTotales, debug);
                lcd_out(4,10,debug);
                //delay_ms(500);*/
                ...
                if(!strncmp(nombreColumnaEscribir, objBaseDatos.nombreColumna, strlen(NombreColumnaEscribir))){
                    ...
                    if(aux > 1){
                        offsetVariasColumnas = (aux) * (MaximoNombreTabla + DOSBYTE);
                    }
                    if(filaEscribir > 0){
                    ...
                    //inttostr(objBaseDatos.filaEscrituraLectura, debug);
                    /*shorttostr(offsetVariasColumnas, debug);
                    lcd_out(4,1,debug);
                    delay_ms(300);*/
...
short escribeBaseDatosNumero(char *nombreTablaEscribir, char *nombreColumnaEscribir, int filaEscribir, long Escritura, short bytes, bool Reemplazar){
    
    unsigned int    direccionFilasProgramadas = 0;
    int             filasProgramadas = 0;
    ...
    if(bytes == BYTE){
        eepromEscribeNumero(encuentraDireccion(nombreTablaEscribir, nombreColumnaEscribir, filaEscribir, &direccionFilasProgramadas), Escritura, bytes);
        if(!Reemplazar){
            filasProgramadas = eepromLeeNumero(direccionFilasProgramadas, DOSBYTE);
            eepromEscribeNumero(direccionFilasProgramadas, filasProgramadas + BYTE, DOSBYTE);
        }
    }
...
long leeBaseDatosNumero(char *nombreTablaLeer, char *nombreColumnaLeer, int filaLeer, short bytes){
    ...
    return Lectura = eepromLeeNumero(encuentraDireccion(nombreTablaLeer, nombreColumnaLeer, filaLeer, &direccionFilasProgramadas), bytes);
}

void escribeBaseDatosChar(char *nombreTablaEscribir, char *nombreColumnaEscribir, int filaEscribir, char *datos, short bytes, bool Reemplazar){
    ...
    eepromEscribeChar(encuentraDireccion(nombreTablaEscribir, nombreColumnaEscribir, filaEscribir, &direccionFilasProgramadas), datos, bytes);
...
void leeBaseDatosChar(char *nombreTablaLeer, char *nombreColumnaLeer, int filaLeer, char *datos, short bytes){
    ...
    eepromLeeChar(encuentraDireccion(nombreTablaLeer, nombreColumnaLeer, filaLeer, &direccionFilasProgramadas), datos, bytes);
}
#endif